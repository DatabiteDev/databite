  ...
  block<TName extends string, TOutput>(
    ...
    builder.blockDefs = [
      ...this.blockDefs,
      {
        ...
        ...(options?.renderConfig && { renderConfig: options.renderConfig }),
      },
    ];
  ...
  build(): Flow<FlowReturnType, Record<string, FlowBlock<any, any>>> {
    ...
    for (const blockDef of this.blockDefs) {
      flowBlocks[blockDef.name] = {
        run: blockDef.run,
        ...(blockDef.requiresInteraction !== undefined && {
          requiresInteraction: blockDef.requiresInteraction,
        }),
        label: blockDef.label,
        ...(blockDef.description && { description: blockDef.description }),
        ...(blockDef.renderConfig && { renderConfig: blockDef.renderConfig }),
      };
    }
